...
public class ModeratedBot extends GPTAgent {
	...
	protected boolean moderate(List<ChatMessage> prompt_list) {
		...
		ModerationRequest moderationRequest = ModerationRequest.builder()
				.input(latest.getContent())
				.model("text-moderation-latest")
				.build();
		
		Moderation modScore = service.createModeration(moderationRequest).getResults().get(0);
		if(!modScore.isFlagged())
			System.out.println("prompt:: " + latest.getContent() + " ~~ is flagged " + modScore.flagged);
		else
			System.err.println("prompt:: " + latest.getContent() + " ~~ is flagged " + modScore.flagged);
	...
	protected boolean moderate(ChatMessage prompt) {
		...
		Moderation modScore = service.createModeration(moderationRequest).getResults().get(0);
		if(!modScore.isFlagged())
			System.out.println("prompt:: " + prompt.getContent() + " ~~ is flagged " + modScore.flagged);
		else
			System.err.println("prompt:: " + prompt.getContent() + " ~~ is flagged " + modScore.flagged);
		return modScore.flagged;
	}
}
